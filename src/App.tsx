...
export default function App() {
  ...
  // Persist user UI profile (metadata only, no auth secrets)
  useEffect(() => {
    try {
      if (currentUser) {
        const safeUser = { ...currentUser, sessionToken: 'CIPHER-HTTPONLY-SESSION' };
        localStorage.setItem('cipher_user', JSON.stringify(safeUser));
      } else {
  ...
  // Synchronize authenticated session from HttpOnly cookie on mount
  useEffect(() => {
    let isMounted = true;
