    ...
    <header className="h-14 border-b border-slate-800/80 bg-[#0a0e17]/95 backdrop-blur-md px-3 sm:px-4 flex items-center justify-between sticky top-0 z-30 select-none">
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              {IS_DEMO_MODE && (
                <span className="text-[9px] sm:text-[10px] font-mono px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40 font-bold uppercase tracking-wider animate-pulse">
                  DEMO MODE
                </span>
              )}
            </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {onOpenSystemStatus && (
          ...
          >
            <span className={`w-2 h-2 rounded-full ${IS_DEMO_MODE ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'}`} />
            <span className="text-slate-400">CONNECTORS:</span>
            <span className={IS_DEMO_MODE ? 'text-emerald-300 font-bold' : 'text-slate-400 font-semibold'}>
              {IS_DEMO_MODE ? '7/8 ONLINE' : '0 configured'}
            </span>
          </button>
        )}
