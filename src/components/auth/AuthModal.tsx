...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleQuickLogin = async (user: UserProfile) => {
    ...
    const demoPassword = 'CipherDemo1234!';
    try {
      let authUser: UserProfile;
      try {
        authUser = await authService.login({
          email: user.email,
        ...
        authUser = await authService.register({
          ...
          organization_name: user.organization.replace(/\s*\(DEMO WORKSPACE\)/i, ''),
          role_name: user.role,
      ...
      setErrorMessage(err.message || 'Quick login failed. Please try manual sign in.');
      setIsLoading(false);
    }
  };
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    if (signupPassword.length < 8) {
      setErrorMessage('Password must be at least 8 characters with upper, lower, digit and symbol.');
      return;
    }
    ...
    try {
      const newUser = await authService.register({
        ...
        organization_name: organization.trim() || 'Global SOC Sentinel Unit',
        role_name: role,
      ...
      setErrorMessage(err.message || 'Failed to provision analyst account.');
      setIsLoading(false);
    }
  };
